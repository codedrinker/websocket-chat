...
function makeWSProxy(onmsg, onopen, onclose, onerror) {
	...
	console.info("��ʼ����:" + beginShakingTime.format("yyyy-MM-dd hh:mm:ss.S"))
	var websocket = (new WebSocket(wsUrl));
	var ws = websocket;
	websocket.onopen = function(evt) {
		var endShakingTime = (new Date());
		console.info("��socket�ɹ�" + evt.data + " ��ʱ��:" + endShakingTime.format("yyyy-MM-dd hh:mm:ss.S"));
		if (onopen) {
			var timecost = endShakingTime.getTime() - beginShakingTime.getTime();
			console.info(timecost + "����")
			onopen(evt);
		}
	};
	websocket.onclose = function(evt) {
		console.info('socket�Ͽ�����' + evt.data + " time:"+ (new Date()).format("yyyy-MM-dd hh:mm:ss.S"));
		if (onclose)
			onclose(evt);
	};
